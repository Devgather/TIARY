...
class UploadPictureTest {
    ...
    @Test
    @DisplayName("[Success] storage url does not end with slash")
    void successIfStorageUrlDoesNotEndWithSlash() {
        // Given
        final AwsStorageProperties awsStorageProperties = new AwsStorageProperties(FactoryPreset.PICTURE.substring(0, FactoryPreset.PICTURE.length() - 1));
        ...
        final Profile defaultProfile = ProfileFactory.createDefaultProfile();

        doReturn(Optional.of(defaultProfile))
                .when(profileRepository)
                .findByUuid(any(String.class));
        ...
        // Then
        assertThat(result.getPicture()).isEqualTo(awsStorageProperties.getUrl() + "/" + picturePathList.get(0));
    }
    ...
    void successIfPictureIsAcceptable() {
        ...
        final Profile updatedProfile = ProfileFactory.create(FactoryPreset.NICKNAME, awsStorageProperties.getUrl() + picturePathList.get(0));

        // When
        final ProfilePictureUploadResponseDto result = profileService.uploadPicture(profileUuid, requestDto);

        // Then
        assertThat(result.getPicture()).isEqualTo(updatedProfile.getPicture());
    }
}