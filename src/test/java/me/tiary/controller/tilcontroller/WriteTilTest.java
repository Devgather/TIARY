...
class WriteTilTest {
    ...
    void failIfProfileUuidDoesNotExist() throws Exception {
        // Given
        ...
        doThrow(new TilException(TilStatus.NOT_EXISTING_PROFILE_UUID))
                .when(tilService)
        ...
        // Then
        resultActions.andExpect(status().is(TilStatus.NOT_EXISTING_PROFILE_UUID.getHttpStatus().value()));

        assertThat(response.getMessages()).contains(TilStatus.NOT_EXISTING_PROFILE_UUID.getMessage());
    }
    ...
    @Test
    @DisplayName("[Success] tag is empty")
    void successIfTagIsEmpty() throws Exception {
        // Given
        final String url = TilApiUrl.TIL_WRITING.getEntireUrl();
    ...
    @Test
    @DisplayName("[Success] tag is blank")
